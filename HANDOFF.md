...
- **Azure Document Intelligence live extraction verified (2026-09-26):** the East US resource endpoint and subscription key are deployed as Worker secrets; `US_ONLY_READING=true` and `AZURE_DI_REGION=eastus` are deployed configuration. A production receipt upload used `azure-document-intelligence` / `prebuilt-receipt`, extracted merchant, date, $64.95 total, and two lines, and passed validation. Typecheck and Azure/US-reader tests pass.
- **Receipt-category repair verified (2026-09-26):** Azure Document Intelligence is factual OCR, so it does not return a bookkeeping category. A conservative local matcher now proposes only an existing client category from unambiguous receipt text; professional approval remains required before it is written. The full production smoke now passes, including Azure extraction, approval, merchant memory, cross-tenant isolation, client portal, and independent zero-residue cleanup.
- **Next:** milestone 8 still awaits the IRS IRIS filer code; Turnstile and Telegram still need their keys.
...
| 11 | US-only receipt reading: Azure Document Intelligence (Textract backup skipped for now: paid after 3 months) | Receipt cap | Built and production-smoke verified 2026-09-26 | complete |
