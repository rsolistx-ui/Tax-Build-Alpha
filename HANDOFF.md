...
- **Smoke status:** the Azure path and synthetic-tenant cleanup are proven (the latest run removed its client and R2 object, then independently verified zero residue). The full smoke remains red at the separate receipt category workflow: after agent approval, the receipt's `category_id` is still unset. Diagnose and repair that workflow before claiming an overall green release. Milestone 8 still awaits the IRS IRIS filer code; Turnstile and Telegram still need their keys.
...
| 11 | US-only receipt reading: Azure Document Intelligence (Textract backup skipped for now: paid after 3 months) | Receipt cap | Configured and live extraction verified 2026-09-26; overall smoke has a separate category-workflow failure | about 1 to 2 hours after key |
