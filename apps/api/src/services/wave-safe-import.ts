...
/**
 * The safe, direct Wave migration service. It deliberately creates only
...
export class WaveSafeImportService {
  ...
  async importCsv(input: { firmId: string; sourceType: ImportType; filename: string; csv: string; defaultClientId?: string | null }): Promise<SafeImportSummary> {
    ...
    for (const [offset, values] of rows.slice(1).entries()) {
      const row = offset + 2;
      try {
        const result = await this.importRow(input, rowObject(headers, values));
  ...
  private async alreadyImported(firmId: string, sourceType: ImportType, sourceKey: string): Promise<boolean> {
    const [row] = await this.db.query<{ id: string }>(
      `SELECT id FROM migration_import_records WHERE firm_id=$1 AND source='wave_csv' AND source_type=$2 AND source_key=$3`,
      [firmId, sourceType, sourceKey],
    );
    return Boolean(row);
  }

  private async record(input: { firmId: string; sourceType: ImportType; sourceKey: string; targetType: string; targetId: string; filename: string }): Promise<void> {
    await this.db.query(
      `INSERT INTO migration_import_records (id, firm_id, source, source_type, source_key, target_type, target_id, source_filename)
       VALUES ($1,$2,'wave_csv',$3,$4,$5,$6,$7)`,
      [newId("mir"), input.firmId, input.sourceType, input.sourceKey, input.targetType, input.targetId, input.filename],
    );
  }

  private async importRow(input: { firmId: string; sourceType: ImportType; filename: string; defaultClientId?: string | null }, row: Record<string, string>): Promise<"created" | "skipped"> {
  ...
  private async customer(input: { firmId: string; sourceType: ImportType; filename: string }, row: Record<string, string>): Promise<"created" | "skipped"> {
    const name = value(row, "name");
    if (!name) throw new Error("Customer name is required");
    const key = await stableKey(`customer:${name.toLowerCase()}`);
    if (await this.alreadyImported(input.firmId, input.sourceType, key)) return "skipped";
  ...
  private async vendor(input: { firmId: string; sourceType: ImportType; filename: string }, row: Record<string, string>): Promise<"created" | "skipped"> {
    const name = value(row, "name");
  ...
  private async account(input: { firmId: string; sourceType: ImportType; filename: string }, row: Record<string, string>): Promise<"created" | "skipped"> {
  ...
  private async invoice(input: { firmId: string; sourceType: ImportType; filename: string; defaultClientId?: string | null }, row: Record<string, string>): Promise<"created" | "skipped"> {
    const number = value(row, "invoice number", "number");
    ...
    const total = asMoney(value(row, "total"));
    const issueDate = asDate(value(row, "invoice date", "issue date"), "Invoice date");
    const dueRaw = value(row, "due date");
    const dueDate = dueRaw ? asDate(dueRaw, "Due date") : issueDate;
    await this.db.transaction([
      { query: `INSERT INTO invoices (id, firm_id, client_id, number, status, issue_date, due_date, subtotal, total, notes, memo) VALUES ($1,$2,$3,$4,'draft',$5,$6,$7,$7,$8,$9)`, params: [id, input.firmId, clientId, number, issueDate, dueDate, total, value(row, "notes") || null, "Imported from Wave — review before sending"] },
      ...
      { query: `INSERT INTO migration_import_records (id, firm_id, source, source_type, source_key, target_type, target_id, source_filename) VALUES ($1,$2,'wave_csv',$3,$4,'invoice',$5,$6)`, params: [newId("mir"), input.firmId, input.sourceType, key, id, input.filename] },
    ]);
    return "created";
  }

  private async bill(input: { firmId: string; sourceType: ImportType; filename: string; defaultClientId?: string | null }, row: Record<string, string>): Promise<"created" | "skipped"> {
    const number = value(row, "bill number", "number");
    ...
    const issueDate = asDate(value(row, "bill date", "issue date"), "Bill date");
    const dueRaw = value(row, "due date");
    ...
    await this.db.transaction([
      ...
      { query: `INSERT INTO bills (id, firm_id, vendor_id, client_id, number, status, issue_date, due_date, total, notes, import_source, import_key) VALUES ($1,$2,$3,$4,$5,'draft',$6,$7,$8,$9,'wave_csv',$10)`, params: [id, input.firmId, vendorId, input.defaultClientId ?? null, number, issueDate, dueDate, asMoney(value(row, "total")), value(row, "notes") || null, key] },
      { query: `INSERT INTO migration_import_records (id, firm_id, source, source_type, source_key, target_type, target_id, source_filename) VALUES ($1,$2,'wave_csv',$3,$4,'bill',$5,$6)`, params: [newId("mir"), input.firmId, input.sourceType, key, id, input.filename] },
    ]);
    return "created";
  }
}
