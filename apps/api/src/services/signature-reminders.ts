...
export async function runSignatureReminders(db: Db, env: Env, deps: { fetch?: typeof fetch } = {}) {
  ...
  for (const row of due) {
    ...
    const link = await issueSigningAccessLink(db, { firmId: row.firm_id, clientId: row.client_id, requestId: row.signature_request_id, recipientEmail: row.taxpayer_email, recipientName: row.taxpayer_name, issuedByUserId: "system:signature-reminder" });
    ...
    const delivered = await sendEmail(env, { to: row.taxpayer_email, fromName: row.firm_name, subject, text, html }, deps);
    if (delivered) sent++;
    await db.query(`INSERT INTO audit_events (id, firm_id, client_id, event, actor_user_id, metadata, created_at) VALUES ($1,$2,$3,'efile_signature_reminder',$4,$5::jsonb,NOW())`,
      [newId("aud"), row.firm_id, row.client_id, "system:signature-reminder", JSON.stringify({ authorizationId: row.id, delivered })]);
  }
