...
export function computeDifAuditReport(data: DifInputData): DifAuditReport {
  ...
  if (grossRev > 0 && mealsRatio > 8.0) {
    ...
    findings.push({
      ...
      explanation: `Meals and dining represent ${mealsRatio.toFixed(1)}% of total gross revenue. The IRS DIF algorithm frequently flags returns where meals exceed 5% of gross revenue for Schedule C examination.`,
...
/**
 * Generates an official Treasury Circular 230 & IRC § 6694 Due Diligence Defense Memorandum.
 */
function generateDefenseMemoText(data: {
  ...
  return `PRE-FILING DUE DILIGENCE & AUDIT DEFENSE MEMORANDUM
Compliance Standard: Treasury Circular 230 § 10.22 & IRC § 6694(a)
Generated by: Truepost Practice OS

EXECUTIVE SUMMARY:
- Pre-Filing IRS DIF Risk Score: ${data.score} / 100 (${ratingLabel})
...
CIRCULAR 230 PRACTITIONER DECLARATION:
In accordance with 31 CFR § 10.22 (Diligence as to Accuracy) and IRC § 6694, the tax preparer has reviewed the taxpayer's books of original entry, verified the deterministic bank reconciliation audit trail, and made reasonable inquiries into anomalous items. All positions taken on the return are supported by substantial authority or have a reasonable basis supported by taxpayer-maintained documentary evidence.`;
}
