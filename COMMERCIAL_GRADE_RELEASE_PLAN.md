...
**In progress, durable email and signing slices released 2026-09-26:** migrations 0081–0087 add a checksum-tracked outbox with idempotency keys, retry backoff, dead-letter and human-superseded states, claim fencing, and owner-only list/replay controls. Support tickets, rule directives, document reminders, and signature follow-ups now record delivery intent with their originating business writes. Signature reminders use a separately versioned encryption key; a row-locked stage operation, a final claim check, and an independent concurrency audit protect staff-issued links, retries, and stale Workers. Email uses the provider idempotency key. Telegram is a separate, durable supplemental alert with at-least-once delivery semantics. The owner dashboard surfaces dead-letter count. Remaining rollout: receipt retries, bank reconnect alerts, and push delivery; add integration coverage against a disposable database and provider failure fixture. During an auth-key rotation, retain `OUTBOX_DELIVERY_LEGACY_AUTH_KEY` until legacy rows drain.
